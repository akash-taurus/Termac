...
// The header button is the last element of the right-aligned header, so its
// hit region must end exactly at the window's right edge on row 0.
func TestAuthButtonHitBounds(t *testing.T) {
	m := DashboardModel{width: 80, userName: "me"}
	w := authButtonWidth(true)
	if !m.headerAuthButtonHit(80-w, 0) {
		t.Fatal("button start column should hit")
	}
	if !m.headerAuthButtonHit(79, 0) {
		t.Fatal("window right edge should hit")
	}
	if m.headerAuthButtonHit(80-w-1, 0) {
		t.Fatal("one column left of the button must miss")
	}
	if m.headerAuthButtonHit(79, 1) {
		t.Fatal("only the header row hosts the button")
	}
}

// A left-click on the Logout button must clear the whole GitHub session and
// schedule deletion of the stored token.
func TestHeaderLogoutButtonClickClearsSession(t *testing.T) {
	m := DashboardModel{
		width:          120,
		viewMode:       ViewLocal,
		selected:       -1,
	...
	model, cmd := m.Update(tea.MouseMsg{Action: tea.MouseActionPress, Button: tea.MouseButtonLeft, X: 119, Y: 0})
...
func TestHeaderButtonClickAwayIsIgnored(t *testing.T) {
	m := DashboardModel{width: 120, viewMode: ViewLocal, selected: -1, userName: "octocat", token: strPtr("ghp_x")}
	if _, cmd := m.Update(tea.MouseMsg{Action: tea.MouseActionPress, Button: tea.MouseButtonLeft, X: 5, Y: 0}); cmd != nil {
		t.Fatal("click away from the button must be ignored")
	}
	model, cmd := m.Update(tea.MouseMsg{Action: tea.MouseActionMotion, Button: tea.MouseButtonNone, X: 119, Y: 0})
	if cmd != nil {
...
func TestHeaderLoginButtonClickOpensModal(t *testing.T) {
	m := DashboardModel{width: 100, viewMode: ViewLocal, selected: -1, tokenInput: textinput.New()}
	model, cmd := m.Update(tea.MouseMsg{Action: tea.MouseActionPress, Button: tea.MouseButtonLeft, X: 99, Y: 0})
	got := model.(DashboardModel)
...
// The header must actually render the button in both auth states.
func TestViewRendersAuthButton(t *testing.T) {
	...
	if !strings.Contains(loggedOut.View(), authButtonLabel(false)) {
		t.Fatalf("header missing the Login button: %q", authButtonLabel(false))
	}
	signedIn := loggedOut
	signedIn.userName = "octocat"
	signedIn.token = strPtr("ghp_x")
	if !strings.Contains(signedIn.View(), authButtonLabel(true)) {
		t.Fatalf("header missing the Logout button: %q", authButtonLabel(true))
	}
}
